    ...
    OwnableUpgradeable
{
    ...
    function settleAuction(
        ...
        // Only settle if desired Noun ID would be minted
        ...
        // Only settle if value is greater than or equal to reserve price
        require(msg.value >= reservePrice, "Below reservePrice");

        // Only settle if parent blockhash matches allowed parent blockhashes
        INounsSeederV2 nounsSeederV2 = INounsSeederV2(address(nounsToken.seeder()));
        uint32 seederUpdateInterval = nounsSeederV2.getUpdateInterval();
        bool matched = false;
        for (uint256 i = 1; i <= seederUpdateInterval; i++) {
            if (blockhash(block.number - i) == expectedParentBlockhash) {
                matched = true;
                break;
            }
        }
        require(matched, "Invalid or expired blockhash");

        // Only settle if value is >= VRGDA price
    ...
    function fetchNextNoun()
        ...
        returns (
            uint256 nounId,
            INounsSeederV2.Seed memory seed,
            string memory svg,
            uint256 price,
            bytes32 hash
        )
    {
        ...
        INounsSeederV2 seederv2 = INounsSeederV2(address(nounsToken.seeder()));
        INounsDescriptor descriptor = INounsDescriptor(nounsToken.descriptor());
        seed = seederv2.generateSeed(_nextNounIdForCaller, descriptor);
